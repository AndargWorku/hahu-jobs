<template>
  <div>
    ...
    <!-- Changed from flex (carousel) to grid (wrapping layout) -->
    <div
        class="grid gap-4 px-4 md:px-0 grid-cols-1 md:grid-cols-2 lg:grid-cols-3"
    >
        <!-- NOTE: Assuming your data is now just a simple array named 'cards' instead of 'infiniteCards' -->
        <div
            v-for="(card, index) in cards" 
            :key="index"
            class="bg-white rounded-xl shadow-sm border dark:bg-[#1B2637] 
           border-gray-200 p-8 flex flex-col justify-between transition-all duration-300 relative group
            " 
